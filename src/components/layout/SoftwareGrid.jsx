            ...
            {isPreviewMode && (
                <div className="bg-blue-600 text-white px-6 py-3 flex items-center justify-between shadow-md z-10 animate-fade-in mb-6 rounded-lg">
                    ...
                    <div className="flex gap-3">
                        <button onClick={onExitPreview} className="px-3 py-1.5 bg-white/20 hover:bg-white/30 rounded text-xs transition-colors">退出预览</button>
                        <button onClick={onImportMerge} className="px-3 py-1.5 bg-white text-blue-600 hover:bg-blue-50 rounded text-xs font-bold transition-colors shadow-sm">一键导入合并</button>
                    </div>
                </div>
            )}
                ...
                <div className="flex items-center gap-2 bg-white p-1 rounded-lg border border-gray-200 shadow-sm">
                    {currentCategory !== 'all' && !searchQuery && (
                        <div className="flex gap-1 border-r border-gray-100 pr-2 mr-2">
                            <button onClick={() => { setEditingCat(data.categories.find(c => c.id === currentCategory)); setModals(prev => ({ ...prev, category: true })); }} className="tooltip-wrap w-8 h-8 flex items-center justify-center rounded hover:bg-gray-100 text-gray-600 transition-colors" data-tip="编辑分类"><Edit3 size={16} /></button>
                            <button onClick={() => { if (confirm('确认删除？')) { saveData({ ...data, categories: data.categories.filter(c => c.id !== currentCategory) }); } }} className="tooltip-wrap w-8 h-8 flex items-center justify-center rounded hover:bg-gray-100 text-red-500 hover:text-red-600 transition-colors" data-tip="删除分类"><Trash2 size={16} /></button>
                        </div>
                    )}
                    {!isPreviewMode && (
                        <>
                            <button onClick={() => { setEditingSoft(null); setModals(prev => ({ ...prev, software: true })); }} className="tooltip-wrap w-8 h-8 flex items-center justify-center rounded hover:bg-gray-100 text-gray-700 hover:text-black transition-colors" data-tip="新增软件"><PlusSquare size={18} /></button>
                            ...
                            <button onClick={() => setModals(prev => ({ ...prev, shareAll: true }))} className="tooltip-wrap w-8 h-8 flex items-center justify-center rounded hover:bg-gray-100 text-gray-700 hover:text-black transition-colors" data-tip="分享整个库"><Share2 size={18} /></button>
                            <button onClick={() => setModals(prev => ({ ...prev, settings: true }))} className={`tooltip-wrap w-8 h-8 flex items-center justify-center rounded hover:bg-gray-100 transition-colors ${wdConfig.autoSync ? 'text-blue-600' : 'text-gray-700 hover:text-black'}`} data-tip="设置 & 同步"><Settings size={18} /></button>
                                ...
                                <SortableItem key={soft.id} id={soft.id} className="h-full" disabled={isDragDisabled}>
                                    <div className="group bg-white p-5 rounded-xl border border-gray-200 hover:border-gray-400 hover:shadow-lg transition-all duration-200 relative flex flex-col h-full select-none">
                                        ...
                                        <div className="flex-grow cursor-pointer" onClick={() => {
                                            ...
                                            <h3 className="font-bold text-gray-900 mb-1 group-hover:text-blue-600 transition-colors truncate">{soft.name}</h3>                                            <p className="text-xs text-gray-500 leading-relaxed line-clamp-2">{soft.description}</p>
                                        </div>
                                        <div className="flex gap-2 mt-4 pt-3 border-t border-gray-50 group-hover:border-gray-100 transition-colors">
                                            ...
                                            {!isPreviewMode && <button onClick={(e) => { e.stopPropagation(); setShareSoft(soft); setModals(prev => ({ ...prev, share: true })); }} className="tooltip-wrap flex items-center gap-1.5 text-xs text-gray-500 hover:text-purple-600 px-2 py-1 rounded hover:bg-purple-50 transition-colors" data-tip="分享软件">
                                                <Share2 size={14} />
                                            </button>}
                                        </div>
                                    </div>
